...
UI_DIR    = HERE / "ui"
LOG_FILE  = Path.home() / "ble_daemon.log"
...
CONFIG_PATH    = Path.home() / ".config" / "m5hub" / "config.json"
PORT           = 7355
RECONNECT_WAIT = 5
CHUNK_SIZE     = 176
ACTIVITY_CAP   = 200
...
async def on_connect_push():
    ...
    await send_cmd({
        ...
        "idle_ms": cfg["idle_ms"],
        "show_stats": 1 if cfg["show_stats"] else 0,
    ...
    if cfg.get("idle_sprite"):
        await send_cmd({"t": "idle", "name": cfg["idle_sprite"]})
...
async def stats_loop():
    while True:
        ...
        try:
            ...
            await send_cmd({"t": "s", "bat": bat_p, "plug": plug, "cpu": cpu_p, "ram": ram_p})
        except Exception as e:
...
async def handle_post_config(request):
    try:
        ...
        cfg  = load_config()
        for k in ("brightness", "idle_ms", "show_stats", "idle_sprite", "volume"):
            if k in body:
                cfg[k] = body[k]
        save_config(cfg)
        await send_cmd({
...
async def handle_sprite_delete(request):
    ...
    sprites.pop(name)
    # If this was the active idle, clear it
    if cfg.get("idle_sprite") == name:
        cfg["idle_sprite"] = ""
        await send_cmd({"t": "idle", "name": ""})
    save_config(cfg)
    push_activity("system", msg=f"Sprite '{name}' deleted")
    return web.json_response({"ok": True})


async def handle_sprite_set_idle(request):
    name = request.match_info["name"]
    cfg  = load_config()
    if name not in cfg.get("sprites", {}):
        return web.json_response({"ok": False, "error": "not found"}, status=404)
    cfg["idle_sprite"] = name
    save_config(cfg)
    await send_cmd({"t": "idle", "name": name})
    push_activity("system", msg=f"Idle sprite set to '{name}'")
    return web.json_response({"ok": True})
...
async def handle_gate(request):
    """Claude Code permission gate. ble_gate.py forwards pre-tool hooks here."""
...
async def main():
    ...
    app.router.add_post  ("/api/sprite/{name}/idle",      handle_sprite_set_idle)
